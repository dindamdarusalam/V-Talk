...
async def http_exception_envelope_handler(request: Request, exc: HTTPException):
    ...
    return JSONResponse(
        ...
        content=error_response(message, code="http_error", details=exc.detail),
        headers=exc.headers,
    )
...
async def translation_socket(websocket: WebSocket):
    ...
    db = SessionLocal()
    try:
        current_user = get_user_from_token(db, websocket.query_params.get("token"))
